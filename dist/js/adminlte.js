...
$("#specialist_photo_").change(function(evt) {
  const [file] = specialist_photo_.files
  if(file) {
    specialist_p_preview_.src=URL.createObjectURL(file)
  }
})
$("#specialist_photo").change(function(evt) {
  const [file] = specialist_photo.files
  if(file) {
    specialist_p_preview.src=URL.createObjectURL(file)
  }
})

$("#flogo").change(function(evt) {
  const [file] = flogo.files
  if(file) {
    flogo_preview.src=URL.createObjectURL(file)
  }
})


// alert("k")
$("#facility_reg_form").submit(function(e) {
  $("#specialist_p_preview").attr("src","")
  $.ajax({
    url: "controller/facility_reg_controller.php",
    type: "post",
   ...
   processData:false,
    beforeSend: function() {
      $("#facility_btn_submit").attr("disabled",true)
      toastr.info("Processing...")
    },
    success: function(data) {
      if(data ==='success') {
        toastr.success("Registered successfully")
        $("#facility_btn_submit").attr("disabled",false)
        $("#facility_reg_form")[0].reset()
        window.location = 'specialist_registration.php'
      }
      else {
        toastr.info(data)
        $("#facility_btn_submit").attr("disabled",false)
...
$("#specialist_reg_form").submit(function(e) {
  $.ajax({
    url: "controller/specialist_reg_controller.php",
    type: "post",
    data: new FormData(this),
    contentType: false,
    cache: false,
    processData: false,
    beforeSend: function() {
      $("#specialist_reg_submit").attr("disabled",true)
      toastr.info("Processing...")
    },
    success: function(data) {
      if(data ==='success') {
        toastr.success("Registered successfully")
        $("#specialist_reg_submit").attr("disabled",false)
        $("#specialist_reg_form")[0].reset()
        get_table('staff_table',facilityid)
...
$("#specialist_reg_form0").submit(function(e) {
  $.ajax({
    url: "controller/specialist_reg_controller.php",
    type: "post",
    data: new FormData(this),
...
$("#patient_form").submit(function(e) {
  $.ajax({
    url: "controller/p_reg_controller.php",
    type: "post",
    data: new FormData(this),
    cache: false,
    processData: false,
    contentType: false,
    beforeSend: function() {
      $("#patient_form_submit").attr("disabled",true)
      toastr.info("Processing...")
    },
    success: function(data) {
      if(data === 'success') {
        toastr.success("Registered successfully")
        $("#patient_form_submit").attr("disabled",false)
        $("#patient_form")[0].reset()
        get_table('patient_table',facilityid)
...
$("#record_form").submit(function(e) {
  $.ajax({
    url: "controller/medical_record_controller.php",
    type: "post",
    data: new FormData(this),
    cache: false,
    processData: false,
    contentType: false,
    beforeSend: function() {
      $("#record_submit").attr("disabled",true)
      toastr.info("Processing...")
    },
    success: function(data) {
      // alert(data)
      if(data === 'success') {
        toastr.success("Registered successfully")
        $("#record_submit").attr("disabled",false)
        $("#record_form")[0].reset()
...
$("#search_patient_form").submit(function(e) {
  $("#preview_patient_modal").modal("show")
  // return false
  $.ajax({
    url: "controller/patient_search_controller.php",
    type: "post",
    data: new FormData(this),
    cache: false,
    processData: false,
    ...
    success: function(data) {
      if(data !== 'Nothing') {
        // let fname = data.split("*")[0]
        // let mname = data.split("*")[1]
        // let lname = data.split("*")[2]
        // let email = data.split("*")[3]
        // let email = data.split("*")[3]
        $("#pat_preview_modal_body").html(`<div class="card card-primary card-outline m-0">
        <div class="card-body box-profile">
          <div class="text-center">
            <img class="profile-user-img img-fluid img-circle" alt="User profile picture" src="uploads/${data.split("*")[7]}">
          </div>

          <h3 class="profile-username text-center">${data.split("*")[0]+' '+data.split("*")[1]+' '+data.split("*")[2]}</h3>

          <p class="text-muted text-center">ID: ${data.split("*")[6]}</p>
          <hr>
          <p class="text-muted text-center">${data.split("*")[3]}</p>
          <p class="text-muted text-center">${data.split("*")[4]}</p>


          <a href="./?pg=p_info&id=${data.split("*")[6]}" class="btn btn-primary btn-block"><b>View Full Information</b></a>
        </div>
        
      </div>`)
      }
      else {
        $("#pat_preview_modal_body").html(`<div class="d-flex justify-content-center align-items-center" style="height:200px;">
        <div class="font-weight-bold">NO Patient with this ID</div>
        </div>`)
      }
      // if(data === 'success') {
      //   toastr.success("Registered successfully")
...
$("#login_form").submit(function(e) {
  $.ajax({
    ...
    success: function(data) {
      if(data ==='success') {
        $("#login_submit").attr("disabled",true)
        window.location="./?pg=med_facility"
      }
      else {
        $("#login_submit").attr("disabled",false)
        toastr.error("Username or password incoreect. Try again.")
      }
    }
  })
})
function navigate(id) {
  window.location=`./?pg=p_info&id=${id}`
}
function get_table(element,data) {
  $.ajax({
    url: element+".php",
    type: "post",
    data: {id:data},
...
// alert("kj")
function open_edit_modal(evt,id) {
  clicked_element = $(evt)
  $("#diagnosis_").html(clicked_element.parents("td").siblings(".t_diagnosis").html())
  $("#treatment_").html(clicked_element.parents("td").siblings(".t_treatment").html())
  $("#prescription_").html(clicked_element.parents("td").siblings(".t_prescription").html())
  $("#test_").html(clicked_element.parents("td").siblings(".t_test").html())
  $("#recordid").val(id)
